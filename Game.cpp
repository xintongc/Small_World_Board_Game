...
vector <Race> Game::initialRaceVector() {
    int temp = 0;
    for (int i = 0; i < 5; i++) {
        if (!raceStack.empty()) {
            temp = raceStack.top();
...
//initially select 5 races and powers to be face-up combos
vector <Power> Game::initialPowerVector() {
    int temp=0;
    for(int i=0;i<5;i++) {
        if (!powerStack.empty()) {
            temp = powerStack.top();
...
vector <int> Game::initialCoinsVector() {
    for(int i=0;i<5;i++){
